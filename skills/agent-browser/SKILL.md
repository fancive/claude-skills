...
For internal sites requiring authentication (`*.baidu.com`, `*.baidu-int.com`, etc.), connect to an existing Chrome session or load saved auth state first. See [references/internal-auth.md](references/internal-auth.md).

```bash
# Connect to Chrome with CDP enabled
...
# Save to ~/.agent-browser/ to avoid accidental commits
...
> ⚠️ **Security**: Never commit auth state files. Use `~/.agent-browser/` or add `*auth*.json` to `.gitignore`.
...
|-----------|-------------|
| [references/internal-auth.md](references/internal-auth.md) | Internal sites (Baidu intranet) authentication via CDP or saved state |
