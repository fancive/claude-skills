...
# Default to ~/.agent-browser/ to avoid accidental commits
STATE_DIR="${HOME}/.agent-browser"
mkdir -p "$STATE_DIR"
STATE_FILE="${2:-${STATE_DIR}/auth-state.json}"

# Security: Warn if state file is in a git repo
if [[ "$STATE_FILE" == "./"* ]] || [[ "$STATE_FILE" != "${HOME}/"* ]]; then
    echo "⚠️  Warning: State file '$STATE_FILE' may be in a git repo."
    echo "   Consider using ~/.agent-browser/auth-state.json to avoid accidental commits."
fi
